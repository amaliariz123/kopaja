    ...
    public function index($range=null)
    {
        ...
        if($range)
        {
                ...
                if($data['daily']->isEmpty()) {
                    ...
                    foreach ($data['daily'] as $item) {
                        if(($item['daily_date'] == $range[1]) || ($item['daily_date'] == $range[2]) ){
                            ...
                            $check = DB::table('daily_reports')->select('daily_date')->pluck('daily_date')->toArray();
                            if((in_array($range[1],$check)==0) && (in_array($range[2],$check)==0)) {
                                $result['premium']=0;
                                $result['regular']=0;
                                $result['kuis']=0;
                                $result['exercise']=0;
            ...
            elseif(((strtotime($range[2]) - strtotime($range[1])) / (24 *3600)+1) < (cal_days_in_month(CAL_GREGORIAN, $month, $year)) && date('m',strtotime($range[1])) == $month && date('m',strtotime($range[2])) == $month){            
                ...
                //custom range in same current month 
                $start = new DateTime($range[1]);
                $interval = new DateInterval('P1D');
                $end = new DateTime($range[2]);
                $end_modify = date_modify($end, '+ 1 day');
                $period = new DatePeriod($start, $interval, $end_modify);
            ...
            elseif(((strtotime($range[2]) - strtotime($range[1])) / (24 *3600)+1) < (cal_days_in_month(CAL_GREGORIAN, date('m',strtotime($range[1])), date('Y',strtotime($range[1])))) && date('m',strtotime($range[1])) == date('m',strtotime($range[2]))){            
            ...
            elseif(((strtotime($range[2])-strtotime($range[1]))/(24*3600)+1) == (cal_days_in_month(CAL_GREGORIAN, date('m',strtotime($range[1])), date('Y',strtotime($range[1])))) && date('m',strtotime($range[1])) == date('m',strtotime($range[2]))){            
                ...
                //custom range in same month but full month
                if ($data['monthly']->isEmpty()) {
                    foreach ($result as $item) {
                        $result['premium']=0;
                        $result['regular']=0;
                    ...
                    foreach ($data['monthly'] as $item) {
                        if ($item['month'] == date('n',strtotime($range[1]))) {
                            $result['premium'] = $result['premium']+$item['total_premium_members'];
                            $result['regular'] = $result['regular']+$item['total_regular_members'];
                            $result['kuis'] = $result['kuis']+$item['total_quizzes'];
                            $result['exercise'] = $result['exercise']+$item['total_tax_exercises'];
                            $result['show'] = date('F, Y', strtotime($range[1]));
                            $result['data'][] = $item;
                            // return $result;
                        }
                    }
            ...
            //last month
            elseif (($range[1] == Carbon::now()->startOfMonth()->subMonth()->toDateString()) && ($range[2] == Carbon::now()->subMonth()->endOfMonth()->toDateString())) {
                if($data['monthly']->isEmpty()){
                    foreach ($result as $item) {
                        $result['premium']=0;
                        $result['regular']=0;
